import { login, loginByToken, logout, getUserInfo } from '@/api/login'
import { getToken, setUser, setToken, removeToken, removeUser, getLocalStorage, setLocalStorage, clearLocalStorage } from '@/utils/auth'
...
const user = {
  state: {
    ...
    roles: [],
    elements: {},
    defaultRouter: '/posture/arrest', // 存放用户默认页面
    setting: {
      articlePlatform: []
    },
    permissionCode: window.config.cityCode,
    permissionMenu: [],
  ...
  mutations: {
    ...
    SET_PERMISSIONCODE: (state, orgCode) => {
      if (orgCode) state.permissionCode = orgCode
    },
    SET_CODE: (state, code) => {
      state.code = code
    },
    ...
    GetUserInfo({ commit, state }) {
      return new Promise((resolve, reject) => {
        ...
        if (user) {
          ...
          commit('SET_PERMISSIONCODE', data.orgCode)
